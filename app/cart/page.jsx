      ...
      <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-8">
            <ul className="space-y-4">
              {cart?.map((item) => (
                <li key={item?.id} className="flex items-center gap-4">
                  <Image
                    src={
                      item?.product[0]?.attributes?.banner?.data?.attributes
                        ?.url
                    }
                    alt={item?.product[0]?.attributes?.title}
                    width={175}
                    height={175}
                    className="size-16 rounded object-cover responsive"
                  />
            ...
            <h2 className="mt-6 text-xl font-bold text-gray-600">Note: All items will be sent via Email</h2>
          </div>
        </div>
      </div>
    </section>
